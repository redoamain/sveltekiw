...
import { getLBMForExport } from "$lib/server/lbm";
import { exportLBMToExcel } from "$lib/export";
import { log } from "$lib/db";

async function doExport(q?: string, tgl1?: string, tgl2?: string, ids?: string[]) {
  const rows = await getLBMForExport({ q: q || undefined, tgl1: tgl1 || undefined, tgl2: tgl2 || undefined, ids });
  if (rows.length === 0) {
  ...
  const xlsx = await exportLBMToExcel(rows, { q, tgl1, tgl2, selectedCount: ids?.length });
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    ...
    log.error({ err, q, tgl1, tgl2 }, "Gagal export LBM");
    return new Response(JSON.stringify({ error: "Gagal mengekspor LBM" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
...
export const POST: RequestHandler = async ({ request, url }) => {
  try {
    ...
    log.error({ err }, "Gagal export LBM POST");
    return new Response(JSON.stringify({ error: "Gagal mengekspor LBM" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
