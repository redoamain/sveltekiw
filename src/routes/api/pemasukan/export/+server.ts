...
import { getPemasukanForExport } from "$lib/server/pemasukan";
import { exportPemasukanToExcel } from "$lib/export";
import { log } from "$lib/db";

async function doExport(q?: string, tgl1?: string, tgl2?: string, ids?: string[]) {
  const rows = await getPemasukanForExport({ q: q || undefined, tgl1: tgl1 || undefined, tgl2: tgl2 || undefined, ids });
  if (rows.length === 0) {
  ...
  const xlsx = await exportPemasukanToExcel(rows, { q, tgl1, tgl2, selectedCount: ids?.length });
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    ...
    log.error({ err, q, tgl1, tgl2 }, "Gagal export pemasukan");
    return new Response(JSON.stringify({ error: "Gagal mengekspor pemasukan" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
...
export const POST: RequestHandler = async ({ request, url }) => {
  try {
    ...
    log.error({ err }, "Gagal export pemasukan POST");
    return new Response(JSON.stringify({ error: "Gagal mengekspor pemasukan" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
