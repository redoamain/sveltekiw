...
async function doExport(q?: string, tgl1?: string, tgl2?: string, ids?: string[]) {
  ...
  const xlsx = await exportTaLogToExcel(rows, {
    q,
    tgl1,
    tgl2,
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    ...
    log.error({ err, q, tgl1, tgl2 }, "Gagal export Log Transaksi");
    return new Response(JSON.stringify({ error: "Gagal mengekspor Log Transaksi" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
...
export const POST: RequestHandler = async ({ request, url }) => {
  try {
    ...
    log.error({ err }, "Gagal export Log Transaksi POST");
    return new Response(JSON.stringify({ error: "Gagal mengekspor Log Transaksi" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
