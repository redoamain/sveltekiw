...
<div class="space-y-5">
	<PageHeader
		title="LBM"
		description="Monitoring Memo In atau Laporan Barang Masuk — filter tanggal & pencarian."
	>
		{#snippet actions()}
	...
	<div class="bg-card overflow-hidden rounded-xl border-[3px] border-border brutal-shadow">
		<div class="bg-card border-b-[3px] border-border flex flex-wrap items-center justify-between gap-3 px-4 py-3">
			...
			<div class="flex items-center gap-2">
				...
				<a
					href={`/api/lbm/export?${new URLSearchParams({ ...(data.q && { q: data.q }), ...(data.tgl1 && { tgl1: data.tgl1 }), ...(data.tgl2 && { tgl2: data.tgl2 }) }).toString()}`}
