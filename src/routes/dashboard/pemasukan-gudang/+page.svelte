<script lang="ts">
	...
	import { Search, Download, Truck } from '@lucide/svelte';
...
<div class="space-y-5">
	<PageHeader
		title="Pemasukan Gudang"
		description="Monitoring Pemasukan Barang Dari Supplier (taTransIHD2) — filter tanggal & pencarian."
	>
		{#snippet actions()}
	...
	<form
		method="get"
		action="/dashboard/pemasukan-gudang"
	...
	>
		...
		<div class="min-w-56 flex-1 space-y-1">
			...
			<div class="relative">
				...
				<Input
					...
					name="q"
					value={data.q}
					placeholder="No Transaksi / Supplier / ItemID / Gudang..."
		...
		{#if data.q || data.tgl1 || data.tgl2}
			<a
				href="/dashboard/pemasukan-gudang"
	...
	<div class="bg-card overflow-hidden rounded-xl border-[3px] border-border brutal-shadow">
		<div class="bg-card border-b-[3px] border-border flex flex-wrap items-center justify-between gap-3 px-4 py-3">
			...
			<div class="flex items-center gap-2">
				...
				<a
					href={`/api/pemasukan/export?${new URLSearchParams({ ...(data.q && { q: data.q }), ...(data.tgl1 && { tgl1: data.tgl1 }), ...(data.tgl2 && { tgl2: data.tgl2 }) }).toString()}`}
			...
			<Table wrapperClass="border-0 shadow-none rounded-none">
				<TableHeader>
					<TableRow class="bg-muted/50">
						...
						<TableHead class="font-mono text-[11px] font-black uppercase tracking-widest">Supplier</TableHead>
					...
					{:else}
						{#each data.rows as r}
							...
							<TableRow class="hover:bg-primary/5 {isChecked ? 'bg-primary/10' : ''}">
								...
								<TableCell class="max-w-44 truncate font-bold text-xs">{r.Supplier ?? '-'}</TableCell>
		...
		<div class="border-t-[3px] border-border bg-muted/30 px-3">
			<Pagination
				...
				total={data.total}
				basePath="/dashboard/pemasukan-gudang"
