...
<div class="space-y-5">
	...
	<form
		method="get"
		action="/dashboard/mutasi-gudang"
	...
	<div class="bg-card overflow-hidden rounded-xl border-[3px] border-border brutal-shadow">
		<div class="bg-card border-b-[3px] border-border flex flex-wrap items-center justify-between gap-3 px-4 py-3">
			...
			<div class="flex items-center gap-2">
				...
				<a
					href={`/api/mutasi/export?${new URLSearchParams({ ...(data.q && { q: data.q }), ...(data.tgl1 && { tgl1: data.tgl1 }), ...(data.tgl2 && { tgl2: data.tgl2 }) }).toString()}`}
			...
			<Table wrapperClass="border-0 shadow-none rounded-none">
				<TableHeader>
					<TableRow class="bg-muted/50">
						...
						<TableHead class="font-mono text-[11px] font-black uppercase tracking-widest">Asal</TableHead>
						<TableHead class="font-mono text-[11px] font-black uppercase tracking-widest">Tujuan</TableHead>
					...
					{:else}
						{#each data.rows as r}
							...
							<TableRow class="hover:bg-primary/5 {isChecked ? 'bg-primary/10' : ''}">
								...
								<TableCell>
									<Badge variant="secondary" class="font-mono text-[10px]">{r.Gudang_Asal ?? '-'}</Badge>
								</TableCell>
								<TableCell>
									<Badge variant="secondary" class="font-mono text-[10px]">{r.Gudang_Tujuan ?? '-'}</Badge>
								</TableCell>
		...
		<div class="border-t-[3px] border-border bg-muted/30 px-3">
			<Pagination
				...
				total={data.total}
				basePath="/dashboard/mutasi-gudang"
