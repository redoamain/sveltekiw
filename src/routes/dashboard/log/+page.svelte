<script lang="ts">
	...
	let allSelected = $derived(
		data.rows.length > 0 && data.rows.every((r) => selected.includes(String(r.TransNo ?? '')))
	);
	...
	function toggleSelectAll() {
		if (allSelected) {
			...
			selected = data.rows.map((r) => String(r.TransNo ?? '')).filter(Boolean);
		}
	}
...
<div class="space-y-5">
	<PageHeader
		title="Log Transaksi"
		description="Monitoring Log Aktivitas & Perubahan Data (taLogNew) — filter tanggal & pencarian."
	>
		{#snippet actions()}
		...
		<div class="min-w-56 flex-1 space-y-1">
			...
			<div class="relative">
				...
				<Input
					...
					name="q"
					value={data.q}
					placeholder="Pengguna / Keterangan / No. Transaksi / Kode Barang / IP..."
	...
	<div class="bg-card overflow-hidden rounded-xl border-[3px] border-border brutal-shadow">
		<div class="bg-card border-b-[3px] border-border flex flex-wrap items-center justify-between gap-3 px-4 py-3">
			...
			<div class="flex items-center gap-2">
				...
				<a
					href={`/api/log/export?${new URLSearchParams({ ...(data.q && { q: data.q }), ...(data.tgl1 && { tgl1: data.tgl1 }), ...(data.tgl2 && { tgl2: data.tgl2 }) }).toString()}`}
			...
			<Table wrapperClass="border-0 shadow-none rounded-none">
				<TableHeader>
					<TableRow class="bg-muted/50">
						...
						<TableHead class="font-mono text-[11px] font-black uppercase tracking-widest">Pengguna</TableHead>
						<TableHead class="font-mono text-[11px] font-black uppercase tracking-widest">Tanggal</TableHead>
						<TableHead class="font-mono text-[11px] font-black uppercase tracking-widest">Jam</TableHead>
						<TableHead class="text-right font-mono text-[11px] font-black uppercase tracking-widest">QTY</TableHead>
						<TableHead class="font-mono text-[11px] font-black uppercase tracking-widest">Keterangan</TableHead>
						<TableHead class="font-mono text-[11px] font-black uppercase tracking-widest">No. Transaksi</TableHead>
						<TableHead class="font-mono text-[11px] font-black uppercase tracking-widest">Kode Barang</TableHead>
						<TableHead class="font-mono text-[11px] font-black uppercase tracking-widest">Tgl Transaksi</TableHead>
						<TableHead class="font-mono text-[11px] font-black uppercase tracking-widest">IP</TableHead>
					</TableRow>
				</TableHeader>
				<TableBody>
						...
						<TableRow>
							<TableCell colspan={10} class="h-24 text-center">
								<p class="font-mono text-xs font-black uppercase tracking-wide text-muted-foreground">
									{data.q || data.tgl1 || data.tgl2 ? 'Tidak ada data untuk filter ini' : 'Belum ada data log'}
								</p>
							</TableCell>
						</TableRow>
					{:else}
						{#each data.rows as r}
							{@const isChecked = selected.includes(String(r.TransNo ?? ''))}
							<TableRow class="hover:bg-primary/5 {isChecked ? 'bg-primary/10' : ''}">
								<TableCell class="text-center">
									<input
										type="checkbox"
										checked={isChecked}
										onchange={() => toggleSelect(String(r.TransNo ?? ''))}
										class="size-4 rounded border-2 border-border text-primary focus:ring-0 cursor-pointer"
									/>
								</TableCell>
								<TableCell class="font-mono text-xs font-black">{r.Username}</TableCell>
								<TableCell class="font-mono text-xs">{r.UserDate ?? '-'}</TableCell>
								<TableCell class="font-mono text-xs">{r.UserTime ?? '-'}</TableCell>
								<TableCell class="text-right font-mono text-xs font-black">
									{r.Kgs != null ? Number(r.Kgs).toLocaleString('id-ID') : '-'}
								</TableCell>
								<TableCell class="max-w-60 truncate text-xs font-bold">{r.Remark || '-'}</TableCell>
								<TableCell class="font-mono text-xs font-black">{r.TransNo ?? '-'}</TableCell>
								<TableCell class="font-mono text-xs">{r.ItemID ?? '-'}</TableCell>
								<TableCell class="font-mono text-xs">
									{r.TransDateTime ? r.TransDateTime.slice(0, 10) : '-'}
								</TableCell>
								<TableCell class="font-mono text-xs">{r.IpAddr ?? '-'}</TableCell>
							</TableRow>
						{/each}
					{/if}
				</TableBody>
