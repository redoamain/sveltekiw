...
const baseSelect = `
    ...
    INNER JOIN [cp].[dbo].[taOpNameODT] AS dt ON hd.[MoveID] = dt.[MoveID] AND hd.[MoveType] = dt.[MoveType]
...
function mapRow(r: Record<string, unknown>): LBKRow {
  return {
...
export async function getLBKPaged(opts: { q?: string; tgl1?: string; tgl2?: string; page: number; pageSize: number }) {
  ...
  const countFrom = hasKeyword
    ? `[cp].[dbo].[taOpNameOHD] hd INNER JOIN [cp].[dbo].[taOpNameODT] dt ON hd.[MoveID]=dt.[MoveID] AND hd.[MoveType]=dt.[MoveType] INNER JOIN [cp].[dbo].[taLocation] g ON hd.[LocID]=g.[LocID] INNER JOIN [cp].[dbo].[taGoods] i ON dt.[ItemID]=i.[ItemID] INNER JOIN [cp].[dbo].[taKindofGoods] kr ON kr.[KodeJenis]=i.[KodeJenis]`
    : `[cp].[dbo].[taOpNameOHD] hd INNER JOIN [cp].[dbo].[taOpNameODT] dt ON hd.[MoveID]=dt.[MoveID] AND hd.[MoveType]=dt.[MoveType]`;
...
export async function getLBKForExport(opts: { q?: string; tgl1?: string; tgl2?: string; ids?: string[] }) {
  if (opts.ids?.length) {
